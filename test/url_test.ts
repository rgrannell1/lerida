...
Deno.test("encodeUrl compresses into a single c= param", () => {
  ...
  assert(!url.includes("markers.0.lat="), "readable params must not leak into the compressed URL");
});
...
Deno.test("the compressed form is shorter than the raw params for a busy map", () => {
  ...
  assert(enc(state).length < raw.length, "compressed URL should be shorter than the raw params");
});
